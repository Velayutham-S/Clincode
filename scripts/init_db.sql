...
CREATE TABLE IF NOT EXISTS code_sets (
    code_set_id    VARCHAR(40) PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS codes (
    code_set_id       VARCHAR(40) NOT NULL
                      REFERENCES code_sets(code_set_id)
                      ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS code_suggestions (
    ...
    span_id          UUID NOT NULL REFERENCES concept_spans(span_id)
                      ON DELETE CASCADE,
    encounter_id     UUID NOT NULL REFERENCES encounters(encounter_id)
                      ON DELETE CASCADE,
    code_set_id      VARCHAR(40) NOT NULL,
...
CREATE TABLE IF NOT EXISTS coder_decisions (
    ...
    suggestion_id    UUID REFERENCES code_suggestions(suggestion_id)
                     ON DELETE SET NULL,
    encounter_id     UUID NOT NULL REFERENCES encounters(encounter_id)
                     ON DELETE CASCADE,
