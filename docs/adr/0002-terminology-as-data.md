...
`code_sets`, `codes`, and `code_relations` are loaded from published release files.

Every suggestion stores `code_set_version`, so an encounter coded in FY2026 can be reproduced exactly even after the FY2027 release lands.

## Consequences

- Annual updates are a data load, not a software release.
