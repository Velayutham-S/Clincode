...
The pipeline is span-first.

`code_suggestions` has a `NOT NULL` foreign key to `concept_spans`.

A model that cannot point at text cannot emit a suggestion; it can only emit a `possible missing documentation` finding instead.
